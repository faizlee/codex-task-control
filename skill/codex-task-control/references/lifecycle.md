...
Registration stores the normalized manifest fields, absolute path, schema version, and SHA-256 digest. Dispatch, progress creation/ingestion, and completion creation/ingestion reread the file and compare the digest. Any change—including a worker changing `errorPolicy`, command text, stage gates, failure mode, or oracle—fails with `IMPLEMENTATION_CONTRACT_DRIFT`; the direct controller must reclaim the task and explicitly bind a new revision rather than silently updating the ledger. Before registration, `audit-implementation-contract` checks structural validity and a narrow set of deterministic cross-command contradictions. Registration repeats error-level checks. Warnings flag controller-review concerns without turning heuristic policy into a hard gate. The control plane does not execute the project commands or decide whether their business output is semantically correct.
...
Before all required stages finish, a worker may write `task_failed` or `task_blocked`. For implementation work, `evidenceCommandId` must name a controller-fixed contract command for `authority=contract_evidence`. `blocking` evidence may stop immediately. `advisory` evidence is always diagnostic. `recoverable` evidence is diagnostic until the event records `recoveryExhausted: true`; only then may ingestion stop the task in `changes_requested`. Missing or unknown command binding also becomes `non_authoritative_diagnostic`: it is appended for investigation but cannot change lifecycle, acceptance, or rework eligibility. Ingestion recomputes this authority from the bound contract instead of trusting a child artifact's declaration.
