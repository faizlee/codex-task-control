...
Each `controllerHeartbeats` entry stores confirmed physical automation identity plus `logicalLeaseDueAt`/`logicalLeaseUpdatedAt`. Protocol v2 also stores pending host action, stale/trigger/delete-failure counters, fuse evidence, bounded history, and retired IDs. Progress can extend the logical lease without replacing an untriggered physical one-shot. Heartbeat state is observability/control-plane evidence, never business lifecycle authority.
...
`changes_requested` is never an execution state. `controller-dispatch-rework` accepts only the first mechanical failure, but only prepares `pendingRework` and a receipt-required host message. It does not increment `attemptCount`. `controller-confirm-rework-dispatched` atomically enters attempt 2 after real delivery; cancellation leaves attempt 1 stopped. `controller-recover-undispatched-attempt` repairs legacy zombie state where `attemptCount > lastDispatchedAttempt`.
...
A meaningful child checkpoint creates `task_progress`. Successful ingestion updates `logicalLeaseDueAt`; while the current physical one-shot has not fired, no App replacement is prepared. Completion and other real boundaries may prepare the next physical cadence. This removes repeated create/switch/delete churn from normal progress.
...
Each repeated stale trigger or failed delete increments bounded evidence and eventually emits one fuse notification. Fusing never fabricates deletion. A stale invocation never processes project queues or rearms. The host-cleanup incident remains visible, but it does not own or block business lifecycle truth.
...
Before all required stages finish, a worker may write `task_failed` or `task_blocked`. For implementation work, `evidenceCommandId` must name a controller-fixed contract command for `authority=contract_evidence`; ingestion then stops the task in `changes_requested`. Missing or unknown command binding becomes `non_authoritative_diagnostic`: it is appended for investigation but cannot change lifecycle, acceptance, or rework eligibility.
