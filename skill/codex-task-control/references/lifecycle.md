...
Every rearm computes a desired state and stores one `pendingAction` with a unique `actionId`, generation `confirmed+1`, a 30-second expiry, and `maxOccurrences=1`. It returns either `create_controller_heartbeat` or `delete_controller_heartbeat` without changing the confirmed generation.
...
If create succeeded but the response was lost, the new automation finds the unique local automation snapshot containing its `actionId`, resolves its own ID, and scans with the pending generation. The scanner returns only `confirm_observed_heartbeat`; confirmation with `observed=true` safely adopts it. If the pending action expired, the still-confirmed old automation receives `compensate_timed_out_heartbeat_action`, records the host timeout, and then may resume a normal scan. This is the compensation path for an App call that hung before its outcome was known.
