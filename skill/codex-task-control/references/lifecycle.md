...
Each registry has `schemaVersion: 1`, `projectKey`, `projectRoot`, `rootControllerThreadIds`, optional `controllerIdentities` and `controllerSuccessions`, `controllerHeartbeats`, `parallelBatches`, `updatedAt`, and `tasks`. New task records include delegation evidence plus `displayKey`, `desiredThreadTitle`, `titleSyncStatus`, `lastSyncedTitle`, `titleSyncError`, `archiveStatus`, `archivedAt`, `archiveError`, append-only `threadActionHistory`, optional schema-v1 `integrationProof`, and schema-v1 lightweight `observabilityReceipts`. `lastDispatchedAttempt` and `lastDispatchedAt` prove that the current work prompt was really sent; title sync alone is not execution evidence. Progress ingestion records `progressEventCreatedAt` and `lastProgressSummary`; completion ingestion records `completionEventCreatedAt` as the notification-failure freshness anchor. Legacy records remain readable; missing identity, observability, or integration-proof fields are not fabricated during read-only access, while partial field groups fail closed. A task's `directControllerThreadId` equals its `parentThreadId`.

## Controller identity and continuity protocol

Long-lived controllers are first-class identities, not worker tasks. Schema-v1 `controllerIdentities` records `controllerThreadId`, `controllerRole=project_controller|topic_controller`, stable `topic` and `stableTitle`, positive `generation`, predecessor/successor links, `active|retired|terminated` status, archive state, and timestamps. `active` identities must be project roots and may not collide with a task. `retired` identities require a registered successor. `terminated` identities require `user_explicit_current_turn`, a reason, and a sealed checkpoint. Identity alone never creates or retains a heartbeat.

An accepted handoff inherits role/topic/title, retires the source, creates the successor at `generation+1`, and keeps the checkpoint digest as continuity evidence. Historical accidents use schema-v1 `controllerSuccessions`: the predecessor must already be a terminal task owned by the caller, the successor must be the fresh visible task returned by Codex “continue in new task”, and the command requires the predecessor's latest sealed checkpoint plus a real host continuation receipt. It adds identity/succession records without changing any old task field or event.

`audit-controller-continuity` is read only. It reports `terminal_controller_without_successor`, `controller_title_repurposed`, and `controller_closed_as_worker`, including controller-shaped legacy tasks that predate identity registration. It may return an identity-scoped archive action only for a retired controller with an accepted successor or a controller explicitly terminated by the current-turn user. Historical violations stay visible after recovery, but `recoveryRequired` becomes false once the successor chain exists.
...
## Adaptive implementation brief and hard-contract escalation
...
New implementation registrations default to `implementationPolicy: adaptive_brief`. The controller uses scope, acceptance, and forbidden decisions to synthesize a schema-v1 brief, or optionally reads `implementation-brief.example.json`. The brief accepts only objective, exploration hints, validation hints, and safety guardrails. It deliberately rejects `allowedWritePaths`, fixed implementation routes, stage gates, and named mandatory validators. The stored digest binds events to the task without turning the brief into a project file lock.

Adaptive workers inspect the real call chain before editing and may select worker-owned progress labels and evidence. They choose headless tests for logic and GUI/MCP/interactive evidence for visible UI. Completion, not registration, is the main evidence boundary: schema-v2 results must report actual affected files, change type, reason, chosen validation, and why it is relevant.

Adaptive registrations also default to `scopePolicy: bounded_incidental`. Scope is the objective and functional boundary, not an exact owning-file forecast. If acceptance exposes a small defect in the same objective and functional domain, the worker can emit a schema-v1 `incidental_repair` receipt and continue without a new objective, replacement, attempt, batch teardown, failure, or fuse increment. The receipt is accepted only while the task is executing its dispatched attempt and proves `localOnly=true`, `reversible=true`, registered conflict domains only, every protected risk flag false, at least one RED reference, and at least one GREEN reference. It records the original blocker, same-objective explanation, actual affected files and reasons, and a written risk assessment. The final result manifest must include every affected path from current-attempt repair receipts.

`scopePolicy: strict_scope` disables this route. Every hard contract is strict. A worker must instead stop and report to the direct controller when a repair crosses a conflict domain, touches a forbidden decision, changes product rules, architecture or trust sources, safety, economy, save data, external effects, irreversible migration, dependencies, or when the boundary cannot be proved. The controller still reviews the actual diff and evidence; the receipt is auditable permission evidence, not self-acceptance.

Result workspace authority is task-specific. When `parallelWorktreeIdentity` is present, `complete` resolves a project-relative manifest against the normalized registered `worktreePath`, never the registry `projectRoot`. It real-paths the manifest and artifact allow roots, verifies the worktree Git top-level exactly matches, verifies main and worktree share the same Git common directory, matches the registered branch, requires candidate commit to equal worktree HEAD, and requires registered base plus last-main-sync commits to be ancestors. Adjacent directories, another repository, symlink/reparse escape, stale branch/base identity, and a different candidate fail closed. The completion snapshot persists `sourceWorkspace`; tasks without worktree identity keep the main project root behavior. Older deliverables without this snapshot remain readable under their original project-root rule.

An old `changes_requested` record caused only by a task-control `tooling` or `contract` protocol defect can use `controller-recover-control-plane-candidate`. Eligibility requires a real dispatch, a registered worktree identity, the authoritative failure for the current attempt, and a currently verifiable frozen candidate/manifest. The direct controller records component `task_control_protocol`, the installed fixed version, a concrete reason, and a real host receipt. The same attempt reopens in `completion_only`; the old failure remains in history and the worker can only submit the exact candidate and manifest again. No business rerun, progress, incidental repair, new failure, scope/forbidden change, candidate drift, mechanical retry, replacement, or objective-fuse cost is permitted.

`hard_contract` is an explicit escalation allowed only for `high_risk_irreversible`, `shared_conflict`, `parallel_coordination`, or `user_explicit`, with a concrete reason. New hard contracts use schema v3. They may freeze real safety or conflict boundaries and may use allowed paths for shared coordination, but they require a validation policy that allows additional and alternative evidence, makes `singleValidatorConclusive=false`, and requires GUI evidence from GUI/MCP/interactive surfaces. Schema-v3 evidence cannot be immediately blocking; it remains recoverable until viable alternative routes are exhausted. A visual presentation gate cannot bind a headless command. Schema-v1/v2 tasks remain read-compatible.

Only hard-contract progress uses ordered stage gates and carry-forward. Only an existing schema-v3 hard contract may be amended. Adaptive work does not acquire a fixed stage sequence during retry, and cannot be silently converted to hard-contract mode while running.
...
Only `integrated`, `blocked`, and `reclaimed` worker tasks enter ordinary archive processing. `controller-scan-events` returns a `set_thread_archived` action only when `archiveStatus` is `pending`, the terminal title is synced, and every visible descendant is already archived. A protected controller identity rejects task completion/review/integration/archive commands. Controller identity archive is separate and becomes pending only after accepted handoff or explicit user-authorized termination. The controller calls `codex_app__set_thread_archived` with `archived: true`, then records success or failure through the matching task or identity command.
...
New `implementation` and `visual_implementation` registrations use `resultProtocolVersion: 2`. Adaptive registrations synthesize result requirements; visual work requires a screenshot/contact sheet and after/current milestone from the `presentation` label. Schema-v3 hard contracts declare equivalent result requirements. Result protocols 0 and 1 remain read-compatible.
...
- every affected project-relative file with `added|modified|deleted|generated` and a reason;
- a validation rationale explaining why the worker-selected logic/UI evidence matches the changed surface;
...
Adaptive failures use `worker_evidence`: the worker records the actual attempted stage, command summary, and evidence refs without claiming a pre-approved validator. Hard-contract failure authority is recomputed from the bound command; recoverable evidence becomes authoritative only after alternative recovery/evidence routes are exhausted. Failure history records every domain, but objective replacement fuses count only authoritative product failures in `test` or `implementation`; `tooling`, `environment`, and `contract` remain control-plane diagnostics.
...
## v0.20 progressive direct-parent context

Ordinary visible workers do not inherit or fork the controller's full conversation. `query-parent` without a context option remains backward compatible and returns only the registered direct-parent ID. `query-parent --context-mode preload` returns a schema-v1 context envelope containing that same identity, the direct parent's verified latest checkpoint preload when available, and a read-only policy. A missing checkpoint returns `checkpointStatus=unavailable` plus `preload=null`; it never blocks dispatch or execution.

The startup preload is exactly the existing checkpoint `preload` projection: only `always` points with `user_confirmed`, `project_fact`, `controller_decision`, or `accepted_result` authority. Candidate, failure, dispute, superseded, raw prompt, response, and tool-output content is not loaded automatically. A nested worker resolves only its immediate registered parent; it does not fall back to a root-controller checkpoint.

`query-parent-context --self <thread> --reason <text>` is a read-only, non-persisting lookup. It returns one host action for the registered direct parent: `read_thread`, initial `turnLimit=3`, and `includeOutputs=false`. The worker starts with completed turn summaries/messages and may use the returned cursor for older turns only while each page provides relevant new evidence. The command does not call the host, write the registry, emit a progress event, create a notification, or retain a heartbeat. If `--point <fact-id>` is supplied, it verifies and returns only that indexed checkpoint point instead of a host action.

Parent history is advisory. It cannot change task scope, forbidden decisions, worktree identity, project rules, accepted checkpoint authority, or lifecycle state. The active parent's unfinished turn is never assumed visible. When the host lacks `read_thread`, or the worker still needs a new controller decision, it uses the existing queued clarification protocol. Full conversation inheritance is reserved for an explicit continuation/handoff design and is never the default parallel-worker route.
