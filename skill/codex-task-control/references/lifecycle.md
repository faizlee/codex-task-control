...
A meaningful child checkpoint creates `task_progress`. Successful ingestion updates `logicalLeaseDueAt`; while the current physical one-shot has not fired, no App replacement is prepared. Once a valid scan records that the one-shot fired, later progress prepares a fresh one-shot instead of extending an already-consumed physical automation. Completion and other real boundaries may also prepare the next physical cadence.
...
This ordering cannot stop the Codex App from spending one model wake before the Skill sees an obsolete scheduled message. v0.15.0 deliberately accepts that bounded cost in exchange for stability: unchanged cycles and failed cleanup stop automatic rearm, business commands stay recoverable, and resumption is explicit. A host-native pre-context hook would remove the extra wake but is no longer required for deadlock/livelock safety.
